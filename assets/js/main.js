...
(function () {
  ...
  function initLanguageSelector() {
    ...
    langList.querySelectorAll("li").forEach((item) => {
      item.addEventListener("click", () => {
        if (currentLang) currentLang.textContent = item.textContent.trim();
        if (currentFlag) currentFlag.src = item.dataset.flag;
    ...
    function hideCurrentLang() {
      ...
      langItems.forEach((li) => {
        if (
          currentLang &&
          li.getAttribute("data-lang") === currentLang.textContent.trim()
        ) {
  ...
  function initSlickSliders() {
    ...
    // Clients Slider - Continuous Auto-scroll
  ...
  $(document).ready(function () {
    ...
    // Progress Bar on Autoplay (Optional Enhancement)
    let progressBar = $(
      '<div class="slider-progress"><div class="progress-fill"></div></div>',
    );
    progressBar.css({
      position: "absolute",
      bottom: "0",
    ...
    progressBar.find(".progress-fill").css({
      width: "0%",
      height: "100%",
    ...
    $(".hero-slider-wrapper").append(progressBar);

    $(".hero-slider").on("beforeChange", function () {
      progressBar.find(".progress-fill").css("width", "0%");
      setTimeout(() => {
        progressBar.find(".progress-fill").css("width", "100%");
      }, 50);
    });
